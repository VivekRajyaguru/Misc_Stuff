...
const GET_USER_DETAILS = "/ibm/cloud/appid/cloudLand/mobile/get_user_details/:id";
...
let selfServiceManager = new SelfServiceManager({tenantId: '01b9184b-952b-4876-9d90-ce1c952d7be4',
iamApiKey: 'ezPPYbpd0JOpMhpB9JzaGYrPYVq4PLdrwXkwHGmM90HF', oauthServerUrl: 'https://appid-oauth.eu-gb.bluemix.net/oauth/v3/01b9184b-952b-4876-9d90-ce1c952d7be4'
...
app.post('/rop/login/submit', function(req, res, next) {
    passport.authenticate(WebAppStrategy.STRATEGY_NAME, function (err, user, info) {
		...
		let emailInputQuery = '&userName=' + req.body.username;
		if (!user) {
...
app.post('/change_password/submit/mobile', passport.authenticate(APIStrategy.STRATEGY_NAME, {session: false}), function (req, res, next) {
	logger.debug('/change_password/submit/mobile');
...
app.get(GET_USER_DETAILS, function(req, res) {
        ...
        selfServiceManager.getUserDetails(req.params.id).then(function (user) {
            console.log(user);
            let inputs = {
                email: user.emails[0].value,
                firstName: user.name && user.name.givenName,
...
function _generateUserScim(body) {
    ...
    userScim.userName = body.userName;
    userScim.role = body.role;
	return userScim;
}
...
app.post(SIGN_UP_SUBMIT, function(req, res) {
	...
	if (!password || !rePassword) {
		...
		if (platform === MOBILE_PLATFORM) {
			console.log("bad sign up input: password can not be empty");
			res.status(400).send("password can not be empty");
		} else {
			_render(req, res, signUpEjs, req.body, language, 'empty_password');
		}
	} else if (!isSamePasswords(password, rePassword)) {
		console.log("Error: password are not the same");
		if (platform === MOBILE_PLATFORM) {
			console.log("bad sign up input: password not the same" );
			res.status(400).send("passwords not the same");
		} else {
		...
		selfServiceManager.signUp(userData, language).then(function (user) {
			...
			if (err && err.code) {
				console.log("error code:" + err.code + " ,bad sign up input: " + err.message);
				if (platform === MOBILE_PLATFORM) {
					res.status(400).send(err.message);
				} else {
	...
	selfServiceManager.getForgotPasswordConfirmationResult(context).then(function (result) {
		...
		if (result && result.success) {
			...
			if (result.error.code === 'NOT_FOUND') {
				...
				console.log('unexpected forgot password result ' + result);
				res.status(500);
				res.send('Something went wrong');
			}
		}
...
app.post(RESET_PASSWORD_SUBMIT, function(req, res) {
	...
	if (!newPassword || !confirmNewPassword) {
		...
		res.status(400).send("password can not be empty");
		    ...
		    console.log('rendering reset password with error: password not the same');
			console.log("bad sign up input: password not the same" );
			res.status(400).send("passwords not the same");
	} else {
		...
		if (codeObject) {
			if (uuid === codeObject.uuid) {
				...
				selfServiceManager.setUserNewPassword(uuid, newPassword, language).then(function (user) {
					console.log('successfully update user password');
					resetPasswordCodesMap.delete(code);
					res.status(200).send(user);
				}).catch(function (err) {
					if (err.code) {
						console.log("error code:" + err.code + " ,bad reset password input: " + err.message);
						res.status(400).send(err.code);
					} else {
						console.log('Error while trying to save user new password: ' + err.message);
						res.status(500).send('Something went wrong');
					}
				});
			} else {
				console.log('The stored code object uuid does not match the passed uuid');
				res.status(500).send('Something went wrong');
			}
		} else {
			console.log('The supplied code was not found in the resetPasswordCodesMap');
			res.status(500).send('Something went wrong');
		}
	}
});
